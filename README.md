...
The Straddle Go library provides convenient access to [the Straddle REST
API](https://docs.straddle.io) from applications written in Go. The full API of this library can be found in [api.md](api.md).
...
```go
params := FooParams{
	Name: straddle.F("hello"),

	// Explicitly send `"description": null`
	Description: straddle.Null[string](),

	Point: straddle.F(straddle.Point{
		X: straddle.Int(0),
		Y: straddle.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: straddle.Raw[int64](0.01), // sends a float
	}),
}
```
...
We also provide a helper `straddle.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   straddle.F("id_xxxx"),
    Data: straddle.F(FooNewParamsData{
        FirstName: straddle.F("John"),
    }),
}
