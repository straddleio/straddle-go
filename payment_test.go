...
func TestPaymentListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Payments.List(context.TODO(), straddle.PaymentListParams{
		...
		HasRefund:         straddle.Bool(true),
		HasResubmit:       straddle.Bool(true),
		IncludeMetadata:   straddle.Bool(true),
		IsRefund:          straddle.Bool(true),
		IsResubmit:        straddle.Bool(true),
		...
		MaxUpdatedAt:      straddle.Time(time.Now()),
		...
		MinUpdatedAt:      straddle.Time(time.Now()),
